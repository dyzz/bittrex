...
func (b *Bittrex) GetHisCandles(market string, interval string, date int64) (candles []Candle, err error) {
	_, presence := CANDLE_INTERVALS[interval]
	if presence == false {
		return candles, errors.New("Wrong interval")
	}

	url := fmt.Sprintf("https://bittrex.com/Api/v2.0/pub/market/GetTicks?marketName=%s&tickInterval=%s&_=%d",
		strings.ToUpper(market), interval, date)
	r, err := b.client.do("GET", url, "", false)
	if err != nil {
		fmt.Println(err)
		return
	}
	var resp Candle_Resp
